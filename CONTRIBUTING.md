...
- Node.js は 24.x のみをサポートします。選定した偶数番号の Active LTS メジャーである Node.js 24 を対象とし、奇数番号、EOL 済み、将来の未検証メジャーは対象外です。
- ローカルの Node.js は [`.node-version`](./.node-version) に合わせてください。Node.js 24 が Maintenance LTS に移行しても次のメジャーへ自動追随せず、Node.js 24 の EOL まで 24.x を維持します。別の偶数番号メジャーへ移行する場合は、検証とレビューを伴う明示的な変更にしてください。`>=24` のような開放的な範囲は指定しません。
