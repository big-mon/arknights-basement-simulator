...
GitHub Actionsの `CI / validate` はPRとmainへのpushで、実行版の一致、frozen install、全テスト、buildを検証します。ローカルと同じarm64の標準macOS 15 runnerを使用し、計算量の多いoptimizerテスト同士のCPU競合を避けるため、CIでは `pnpm test --no-file-parallelism --testTimeout=30000` を使用します。共有runnerでの機能テストの待機時間を30秒とし、明示的な計算時間上限のassertionは維持します。
